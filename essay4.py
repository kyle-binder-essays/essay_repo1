...
def define_input_parameters():
    ...
    cwd = os.getcwd()
    csv_monthly = cwd + str('\DATA\FRED_Monthly_Yields\monthly_yields_1962.CSV')
    df_monthly = pd.read_csv(csv_monthly,skiprows=0,nrows=685-1,index_col=0)
    
    csv_daily = cwd + str('\DATA\FRED_Daily_Yields\daily_yields_1962.CSV')
    ...
    return df_daily, df_monthly, shorter_mat_yrs, longer_mat_yrs
...
def main():
    ...
    yields_daily, yields_monthly, \
    shorter_mat_yrs, longer_mat_yrs = define_input_parameters()
    ...
    return  yields_daily, yields_monthly, \
            interp_yields_daily, interp_yields_monthly, \
            returns_daily, returns_monthly, \
            price_carry_return_d, coupon_return_d, total_return_d, \
            price_carry_return_m, coupon_return_m, total_return_m, \
            price_sold_at_m, price_sold_at_d, \
            left_fraction_d_tt, right_fraction_d_tt
            
# Desired output variable used in Essay #4 is "returns_daily":
yields_daily, yields_monthly, interp_yields_daily, interp_yields_monthly, \
returns_daily, returns_monthly, \
price_carry_return_d, coupon_return_d, total_return_d, \
price_carry_return_m, coupon_return_m, total_return_m, \
price_sold_at_m, price_sold_at_d, \
left_fraction_d_tt, right_fraction_d_tt = main()
