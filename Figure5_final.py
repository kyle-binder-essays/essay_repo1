...
def plot_tstats_chart(row_of_tstats):

    fig = plt.figure(figsize=(8,12))
    ax1 = fig.add_subplot(211)

    fig.suptitle('Figure 5')
    ax1.title.set_text('T-Statistics, 1926-2021')
    ax1.set_xlabel('Lag (Months)')
    ax1.set_ylabel('T-Statistics (Red Line: +/-1.96)')
    ax1.set_ylim(-4, 6)

    ax1.axhline(y=1.96, color='r', linestyle='-')
    ax1.axhline(y=-1.96, color='r', linestyle='-')
    
    # For Panel A, make the major tick label in multiples of 2.0, and
...
plot_tstats_chart(row_of_tstats_same_history)
