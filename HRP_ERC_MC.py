...
def getERC(cov,corr):
    # Construct an ERC portfolio
    corr,cov=pd.DataFrame(corr),pd.DataFrame(cov)
    assets_risk_budget = [float(1) / float(cov.shape[1])] * cov.shape[1]
    init_weights = [float(1) / float(cov.shape[1])] * cov.shape[1]
